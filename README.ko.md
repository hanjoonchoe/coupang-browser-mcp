...
| `proceed_to_checkout` | — | 주문서를 열고 **멈춤** — 탭은 열어두고, 결제 버튼은 절대 누르지 않음 |

## 안전 모델

1. **읽기는 자유롭게, 쓰기는 확인 후.** 장바구니 변경은 미리보기를 반환하고, 두 번째 호출에서 `confirm=true`가 있어야 실행됩니다.
2. **돈은 저절로 움직이지 않습니다.** 결제는 주문서에서 멈춥니다.
